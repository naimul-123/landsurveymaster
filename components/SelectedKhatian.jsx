...
const SelectedKhatian = ({ khatian, idx, selectedKhatians, setSelectedKhatians }) => {
    const [landAmountType, setLandAmountType] = useState('portionBased')
    console.log(landAmountType);
        ...
        return khatians.map(khatian => {
            ...
            const calculateTransferable = (transferableArray, basePlots) => {
                return (transferableArray || []).map(item => {
                    const matchedPlot = basePlots.find(plot => plot.plot_no === item.plot_no);
        ...
        const updatedKhatians = selectedKhatians.map(khatian => {
            ...
            updatedKhatian.owners = (updatedKhatian.owners || []).map(owner => {
                if (owner._id === ownerId) {
                    return {
                        ...owner,
                        isTransferor: true, // এখানে মার্ক করলাম
                    };
                }
                return owner;
            });
        ...
        const updatedKhatians = selectedKhatians.map(khatian => {
            ...
            updatedKhatian.owners = (updatedKhatian.owners || []).map(owner => {
                if (owner._id === ownerId) {
                    ...
                    return {
                        ...rest, // isTransferor বাদ দিয়ে দেবো
                    };
                }
                return owner;
            });
    ...
    return (
        <div key={khatian._id} className="bg-white overflow-hidden">
            ...
            <div className="bg-gray-300 p-4 font-extrabold text-gray-800 ">
                <h2 className='text-xl md:text-2xl text-center  badge badge-lg bg-inherit w-max mx-auto '>{idx + 1} নং তফসিল:-</h2>
                <h2 className=" ">
                    জেলা: {khatian?.district} | থানা: {khatian.thana} | মৌজা: {khatian.mouja} | {khatian.khatian_type} | খতিয়ান নং: {khatian?.khatian_No}
                </h2>
            </div>
            ...
            <div className="flex gap-4 p-4 bg-gray-200">
                ...
                <div className='flex-1/2'>
                    <h3 className="text-xs font-bold mb-2 border-b">দাগ সিলেক্ট/ডিসিলেক্ট করুন</h3>
                    <ul className="flex flex-wrap gap-2">
                        {khatian.plots.map(plot => (
                            <li key={plot.plot_no}>
                                <button
                                    onClick={() => handleTogglePlotSelection({ khatianId: khatian._id, plot_no: plot.plot_no })}
                                    className={`btn btn-xs ${plot.isSelected ? 'btn-primary' : 'btn-outline btn-primary'}`}
                                >
                                    {plot.plot_no}
                                </button>
                            </li>
                        ...
                        {khatian.plots.length > 1 && (
                            <>
                                <li>
                                    <button onClick={() => handleAddAllPlots(khatian._id)} className="btn btn-xs btn-outline btn-success">Select All</button>

                                </li>
                                <li>

                                    <button onClick={() => handleRemoveAllPlots(khatian._id)} className="btn btn-xs btn-outline btn-success">Deselect All</button>
                                </li>
                            </>
                        )}
                    </ul>
                ...
                <div className='flex-1/2'>
                    <h3 className="text-xs font-bold mb-2 border-b">রসদীয় জমি নির্বাচনের ধরণ</h3>
            ...
            <div className="flex gap-4 p-4 bg-gray-200   ">
                ...
                <div className='flex-1/2'>
                    ...
                    <div className="space-y-1">
                        {
                            khatian.owners?.filter((owner) => !owner.isTransferor).map((owner, idx) =>

                                <OwnerCollapse key={idx} khatian={khatian} owner={owner} idx={idx} selectedKhatians={selectedKhatians} handler={handleaddOwner} landAmountType={landAmountType} setLandAmountType={setLandAmountType} />
                ...
                {/* নির্বাচিত মালিক */}
                <div className='flex-1/2'>
                    <h3 className="text-lg font-semibold text-blue-600 mb-2 border-b">নির্বাচিত দাতাগণ</h3>
                    <ul className="space-y-1">
                        {khatian?.owners?.filter(owner => owner.isTransferor).map((owner, idx) => {
                            const totalSelectedTransfarableLand = owner.transferable?.reduce((acc, plot) => {
                                const matchedPlot = khatian.plots.find(p => p.plot_no === plot.plot_no);
                                if (matchedPlot) {
                                    return acc + Number(plot.transferableTotalLand || 0);
                                }
                                return acc;
                            }, 0);

                            return owner.transferable?.length > 0 && (
                                <li key={owner._id} className="bg-gray-100 p-2 rounded flex justify-between items-center">
                                    <span>{idx + 1}. {owner.name} হস্তান্তরযোগ্য জমি: {Number(totalSelectedTransfarableLand).toFixed(3)}</span>
                                    <button
                                        onClick={() => handleRemoveOwner({ khatianId: khatian._id, ownerId: owner._id })}
