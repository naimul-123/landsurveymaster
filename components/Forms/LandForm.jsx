...
export default function LandForm() {
  ...
  const handleOwnerChange = (index, field, value) => {
    if (field === "share") {
      ...
      const newTotalShare = Number(
        (otherSharesTotal + Number(value || 0)).toFixed(10)
      );
      ...
      const totalShare = landInfo?.owners?.reduce(
  ...
  const handlePlotChange = (index, field, value) => {
    ...
    const totalLand = plots.reduce(
      (total, plot) => total + plot.totalLandInPlot * plot.share,
      0
    );
  ...
  const removePlot = (index) => {
    ...
    const totalLand = updatedPlots.reduce(
      (total, plot) => total + plot.totalLandInPlot * plot.share,
      0
    );
  ...
  const removeOwner = (index) => {
    ...
    const totalShare = updatedOwners.reduce(
      (total, owner) => total + Number(owner.share || 0),
      0
    );
    setOwnersTotalShare(Number(totalShare.toFixed(10)));
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <h2 className="text-2xl font-bold col-span-full text-primary">
          খতিয়ানের তথ্য
        </h2>
        ...
        <div className="col-span-full mt-4">
          ...
          <div className="flex flex-wrap items-center gap-4">
            <span
              className={
                !isDecimal ? "font-bold text-blue-600" : "text-gray-400"
              }
            >
              প্রাচীন(আনা-গন্ডা)
            </span>
            ...
            <span
              className={
                isDecimal ? "font-bold text-green-600" : "text-gray-400"
              }
            >
              দশমিক ভিত্তিক
            </span>
      ...
      <div
        tabIndex={0}
        className="collapse collapse-arrow bg-base-100 border border-base-300 rounded-box"
      >
        <input type="checkbox" />
        <div className="collapse-title font-semibold">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-2 mt-2 text-sm">
            <h2>
              অন্তর্ভূক্ত দাগ:{" "}
              {landInfo?.plots?.map((plot) => plot.plot_no).join(", ")}
            </h2>
                ...
                <tr key={index}>
                  ...
                  <td>
                    <input
                      ...
                      value={plot.plot_no}
                      onChange={(e) =>
                        handlePlotChange(index, "plot_no", e.target.value)
                      }
      ...
      >
        ...
        <div className="collapse-title font-semibold">
          ...
          <div className="flex flex-col md:flex-row justify-between gap-2 mt-2 text-sm">
            ...
            <h2>
              বন্টনকৃত জমি: {(ownersTotalShare * landInfo.totalLand).toFixed(4)}
            </h2>
          </div>
        </div>
      ...
      <div className="text-center">
        <button
          type="submit"
          className="btn btn-success text-white px-6 py-2 mt-6"
        >
          সাবমিট করুন
        </button>
      </div>
