...
const TransferLand = () => {
    ...
    const handleAddKhatian = async (e) => {
        ...
        if (!prevIds.includes(selectedKhatianId)) {
            const selectedKhatian = khatianinfo.find(k => String(k._id) === selectedKhatianId);
            if (!selectedKhatian) return alert("Invalid Khatian selected");

            try {
                const { owners } = await getData(`/api/getowners?khatianId=${selectedKhatianId}`);
                if (owners.length > 0) {
    ...
    const handleAddNewBuyer = async (e) => {
        ...
        const { owner } = await postData('/api/addowner', { name: buyerName, khatianIds, alive: true });
        console.log(owner);
        if (owner) {
            setSelectedKhatians(prev =>
                prev.map(k => {
    ...
    return (
        <div className="space-y-6">
            ...
            <form onSubmit={handleAddKhatian} className="flex flex-wrap items-center gap-4 bg-white shadow-md p-4">
                <select defaultValue="" name="khatianId" className="select select-bordered w-full md:w-auto">
                    <option value="">একটি খতিয়ান সিলেক্ট করুন</option>
                    {khatianinfo?.map(khatian => (
                        <option key={khatian._id} value={khatian._id}>
                            জেলা: {khatian?.district} থানা: {khatian.thana} মৌজা:{khatian.mouja} {khatian.khatian_type} {khatian?.khatian_No} নং খতিয়ান
                        </option>
                    ))}
                </select>
                ...
                selectedKhatians.length > 0 && <>
                    <div className="space-y-8">
                        {selectedKhatians?.map((khatian, idx) =>
                            ...
                            <SelectedKhatian key={idx} khatian={khatian} idx={idx} selectedKhatians={selectedKhatians} setSelectedKhatians={setSelectedKhatians} />
                        )}
                    </div>
                    <div className="flex gap-4 p-4 bg-gray-200   ">

                        <div className='flex-1/2'>
                            <h3 className="text-lg font-semibold text-info mb-2 border-b">গ্রহীতা যোগ করুন</h3>
                            <form className='' onSubmit={handleAddNewBuyer}>
                                <fieldset className="fieldset">
                                    <legend className="fieldset-legend">নতুন গ্রহীতা:</legend>
                                    <div className='flex gap-2'>
                                        <input type="text" name='buyerName' className="input input-sm input-info" placeholder="মালিকের নাম, পিং/জং" />
                                        <button className='btn btn-info btn-soft btn-sm'>যোগ করুন</button>
                                    </div>
                            ...
                            {potentialBuyers.length > 0 &&
                                <ul className="space-y-1 my-2">
                                    <h3 className="text-xs font-bold mb-2">মালিকগণের মধ্য থেকে নির্বাচন করুন</h3>
                                    {potentialBuyers?.map((buyer, idx) => {


                                        return (
                                            <li key={buyer._id} className="bg-gray-100 p-2 rounded flex justify-between items-center">
                                                <span>{idx + 1}. {buyer.name}</span>
                                                {/* এখানে দুইটি কন্ডিশন হবে, যদি buyer.acquired?.length=== 0 অথবা buyer.acquired না থাকে */}
                                                {(!buyer.acquired || buyer.acquired.length === 0) &&
                                                    <button
                        ...
                        {/* নির্বাচিত মালিক */}
                        <div className='flex-1/2'>
                            <h3 className="text-lg font-semibold text-blue-600 mb-2 border-b">নির্বাচিত গ্রহীতাগণ</h3>
